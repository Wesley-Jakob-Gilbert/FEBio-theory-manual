...
| Section | Title | Inline formulas | Display formulas | Citations | Figures | Converted cleanly? | Needs manual review |
|---|---|---|---|---|---|---|---|
| A.1 | Second-Order Tensors | 230 | 83 | 0 | 2 | Mostly | `subsubsec:determinant`, referenced cross-chapter from Section 2.5, is defined here; 2 figures (`FigOrthoBases.png`, `FigRotationAboutX3.png`) fetched from upstream; 10 raw `Example` layouts in the source collapse to 5 real examples (3 runs of consecutive `Example` layouts are one logical example each) plus 2 `Theorem*` layouts — hand-patched in the generated `.md`, not yet fixed at the converter level; see "Specific insets flagged" below |
...
| A.1 | 3 places where the source has **multiple consecutive `\begin_layout Example` blocks with nothing in between** ("Scaling transformation" x4, "Show that..."/"Using indicial notation..." x2, "Rotation about $x_3$"/"Reflection about..." x2) | **Real converter gap, not yet fixed at the source**: in LyX/LaTeX's `theorems-ams` module, consecutive same-style paragraphs continue the *same* numbered environment (extra paragraphs of one example), they don't each start a new one — a rule the counter/renderer don't currently implement (it increments and starts a fresh bold label on every `Example` layout unconditionally, regardless of what preceded it). This produced 4 (then unmerged: 2, 2) separate "Example N." labels in a row for what the published manual presents as one example each, e.g. "Example 2. A scaling transformation... Example 3. Solution. Is T a tensor?..." reading as 4 disconnected examples instead of one problem-then-solution. Found by systematically walking the parse tree for every consecutive same-kind `Example`/`Theorem*` run in the whole document (confirmed the 2 in 7.11 and the other 2 A.1/A.2 instances are each genuinely standalone, not affected) | **Hand-patched directly in `docs/theory/chapterA/A.1-second-order-tensors.md`** (merged the 4/2/2 runs into single "Example N." blocks and renumbered 1-5 accordingly) rather than fixed in `tools/lyx2md.py`, per explicit request to patch this one file for now. This is **not** converter-level: rerunning `python3 build.py` will regenerate this file from source and silently drop the hand-patch, reintroducing the split-example bug. A real fix belongs in `render_section_body()`'s `Example`/`Theorem*` branch (and the matching pre-scan counter): only increment/relabel when the *previous* top-level layout item wasn't the same kind; otherwise append the paragraph to the current example's body instead of starting a new one |
