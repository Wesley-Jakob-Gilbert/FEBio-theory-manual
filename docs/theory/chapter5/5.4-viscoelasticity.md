...
A generalization of Fung's earlier continuous relaxation spectrum may be derived from the work of Malkin [^5.4-18] who proposed to use a function proportional to $\tau^{-\beta}$. If we constrain this spectrum to the range $\tau_{1}\le\tau\le\tau_{2}$ it takes the form (Figure [2](#fig:Malkin-relaxation-spectrum))
...
When substituted into \eqref{eq:rrf-g-continuous-spectrum} this continuous relaxation spectrum produces the reduced relaxation function (Figure [3](#fig:Malkin-reduced-relaxation))
...
Another example for a continuous relaxation spectrum is the exponential spectrum (Figure [4](#fig:exponential-relaxation-spectrum))
