...
where $d\mathbf{x}_{m}=ds_{m}\mathbf{m}$ and $d\mathbf{x}_{n}=ds_{n}\mathbf{n}$. Note that we may write $\mathbf{m}\cdot\mathbf{n}=\cos\theta=\sin\gamma$, where $\theta$ is the angle between $d\mathbf{x}_{m}$ and $d\mathbf{x}_{n}$ and $\gamma=\frac{\pi}{2}-\theta$ is its complementary angle (Figure [2](#fig17)), so that
...
according to the definition of the determinant given in Section [Determinant of a Tensor](../chapterA/A.1-second-order-tensors.md#subsubsec:determinant).
