...
Now, we need to make constitutive assumptions on the dependent variables which appears in Clausius-Duhem inequality, eq.\eqref{eq:Clausius-Duhem-inequality}, namely, $\psi$, $\eta$, $\boldsymbol{\sigma}$ and $\mathbf{q}$. These functions could depend on a number of _state variables_, such as strain, rate of deformation, temperature, temperature gradient, etc. For example, the pressure in a gas is generally assumed to depend on temperature and density, so it would follow that $\boldsymbol{\sigma}$ for a gas should exhibit the same dependencies.

For solid materials we have found that that the mass density is dependent on the strain (eq.\eqref{eq88}, $\rho=\rho_{r}\left(\det\mathbf{C}\right)^{-1/2}=\rho_{r}/J$, where $\rho_{r}$ is a constant); so long as a dependence on the strain is provided, an explicit dependency on mass density is not required.
...
where we have made use of eq.\eqref{eq:Edot-D-relation}. Making use of the identity
...
Then, using eq.\eqref{eq88} and recalling that $\rho_{r}$ is constant, eq.\eqref{eq103-1} reduces to
...
where $da$ is an elemental area of $S$ in the current configuration. Here, we used the relation between the traction vector $\mathbf{t}$ and the Cauchy stress $\boldsymbol{\sigma}$, given in eq.\eqref{eq:traction-stress-relation}. Note that the directed area $d\mathbf{a}=\mathbf{n}\,da$ in this integral may be related to $d\mathbf{A}$ in the reference configuration, using Nanson's formula in eq.\eqref{eq:Nanson-formula} to produce
...
Since $J=\left(\det\mathbf{C}\right)^{1/2}$, the incompressibility constraint of eq.\eqref{eq:incompressibility-constraint} implies that the symmetric strain tensor $\mathbf{E}=\frac{1}{2}\left(\mathbf{C}-\mathbf{I}\right)$ used as a state variable in the derivation of the hyperelasticity relation (see Section [Constitutive Restrictions](#subsec:Constitutive-Restrictions)) does not truly have six independent components. Therefore, we cannot claim that $\mathbf{E}$ can vary arbitrarily when enforcing the entropy inequality. Instead, we introduce the incompressibility constraint of eq.\eqref{eq:incompressibility-redux} into the Clausius-Duhem inequality, eq.\eqref{eq:Clausius-Duhem-inequality}, using the method of Lagrange multipliers,
